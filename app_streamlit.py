...
Cada nó valida e assina digitalmente os blocos propostos. 
...
if "nos" not in st.session_state:
    ...
    st.session_state["mostrar_web3"] = False  # inicializa Web3 desativado
...
# ============================================================
# ABA PRINCIPAL — SIMULAÇÃO DE CONSENSO
# ============================================================
with tab_main:
    ...
    if validar_consenso(nos):
        st.success(f"Todos os nós estão sincronizados e íntegros. Blockchain com **{len(next(iter(nos.values())))}** blocos.")
    else:
    ...
    with st.container(border=True):
        ...
        for i, (nome, df) in enumerate(nos.items()):
            with cols[i]:
                st.metric(label=f"{nome}", value=f"{df.iloc[-1]['hash_atual'][:12]}...", delta=f"Blocos: {len(df)}")
    ...
    with st.expander("Propor Novo Bloco e Iniciar Votação", expanded=True):
        ...
        with col_prop:
            propositor = st.selectbox("Selecione o nó propositor:", list(nos.keys()), key="select_propositor_main")
        with col_quorum:
            quorum = st.slider("Defina o quorum mínimo:", 1, len(nos), 2, key="slider_quorum_main")
    ...
    if st.session_state.get("mostrar_web3", False):
        with st.expander("Visualização Web3 (Simulada)", expanded=True):
            mostrar_demo_web3(
                st.session_state.get("web3_evento_texto", ""),
                st.session_state.get("web3_hash", "")
            )

            if st.button("Ocultar Web3", key="ocultar_web3"):
    ...
    with st.container(border=True):
        ...
        with col_audit_actor:
            audit_actor = st.selectbox("Ator/Fonte do Log:", ["Usuário-Streamlit", "Sistema", "Nó de Validação"], key="audit_actor")
        with col_audit_message:
            audit_message = st.text_input("Mensagem de Auditoria (Ação):", "Evento de teste manual disparado.", key="audit_message")
        ...
        with col1:
            if st.button("Carregar da Nuvem", use_container_width=True):
                ...
                if df is not None:
                    ...
                    nos["Nó_A"] = df
                    st.success("Blockchain carregada e Nó_A atualizado da nuvem!")
                else:
        ...
        with col2:
            if st.button("Salvar Manualmente", use_container_width=True):
                try:
                    salvar_blockchain_firestore(nos["Nó_A"])
